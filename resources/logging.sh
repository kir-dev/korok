...
/subsystem=logging/periodic-rotating-file-handler=PEK-WARN:add(append=true, autoflush=true,named-formatter=PATTERN, file={relative-to="pek.log.dir", path="pek-warn.log"}, level=WARN, suffix=".yyyy-MM-dd", filter-spec="level(WARN)")
/subsystem=logging/periodic-rotating-file-handler=PEK-ERROR:add(append=true, autoflush=true,named-formatter=PATTERN, file={relative-to="pek.log.dir", path="pek-error.log"}, level=ERROR, suffix=".yyyy-MM-dd", filter-spec="level(ERROR)")
